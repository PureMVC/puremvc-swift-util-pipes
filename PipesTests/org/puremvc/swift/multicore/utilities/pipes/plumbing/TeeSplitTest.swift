...
class TeeSplitTest: XCTestCase {
    ...
    */
    func testConnectingAndDisconnectingIOPipes() {
        ...
        XCTAssertTrue(teeSplit.disconnect() as! Pipe === pipe4 as! Pipe, "Expecting disconnected pipe 4")
        XCTAssertTrue(teeSplit.disconnect() as! Pipe === pipe3 as! Pipe, "Expecting disconnected pipe 3")
        XCTAssertTrue(teeSplit.disconnect() as! Pipe === pipe2 as! Pipe, "Expecting disconnected pipe 2")
        XCTAssertTrue(teeSplit.disconnect() as! Pipe === pipe1 as! Pipe, "Expecting disconnected pipe 1")
        XCTAssertNil(teeSplit.disconnect() as? Pipe, "Expecting nil")
    }
    ...
    */
    func testDisconnectFitting() {
        ...
        pipe2.connect(PipeListener(context: self, listener: self.callBackMethod))
        pipe3.connect(PipeListener(context: self, listener: self.callBackMethod))
        ...
        // test assertions
        XCTAssertTrue(teeSplit.disconnectFitting(pipe4) as! Pipe === pipe4 as! Pipe, "Expecting teeSplit.disconnectFitting(pipe4) === pipe4")
        XCTAssertNil(teeSplit.disconnectFitting(pipe4) as? Pipe, "Expecting teeSplit.disconnectFitting(pipe4) == nil")
        
        // Write a message to the tee
        teeSplit.write(Message(type: Message.NORMAL))
