...
class FilterTest: XCTestCase {
    ...
    */
    func testConnectingAndDisconnectingIOPipes() {
        ...
        // test assertions
        XCTAssertNotNil(pipe1, "Expecting pipe1 is not nil")
        XCTAssertNotNil(pipe2, "Expecting pipe2 is not nil")
        XCTAssertNotNil(filter, "Expecting filter is not nil")
        ...
        XCTAssertNotNil(disconnectedPipe, "Expecting disconnected pipe2 from filter not nil")
    }
    ...
    */
    func testFilteringNormalMessage() {
        ...
        let message: IPipeMessage = Message(type: Message.NORMAL, header: Rectangle(width: 10, height: 2))
        ...
        let filter = Filter(name: "scale", output: PipeListener(context: self, listener: self.callBackMethod),
            filter: { (message: IPipeMessage, params: Any?) -> Bool in
                ...
                let rectangle = message.header as! Rectangle
        ...
        let received = messagesReceived.remove(at: 0)
        ...
        XCTAssertTrue((received.header as! Rectangle).width == 100, "Expecting received.Rectangle['width'] == 100")
        XCTAssertTrue((received.header as! Rectangle).height == 20, "Expecting received.Rectangle['height'] == 20")
    }
    ...
    */
    func testBypassAndFilterModeToggle() {
        ...
        XCTAssertTrue((received1.header as! Rectangle).height == 2, "Expecting received1.header['height'] == 2")
        ...
        XCTAssertTrue((received2.header as! Rectangle).height == 20, "Expecting received2.header['height'] == 20")
    }
    ...
    */
    func testSetParamsByControlMessage() {
        // create messages to send to the queue
        let message = Message(type: Message.NORMAL, header: Rectangle(width: 10, height: 2))
        
        // create filter, attach an anonymous listener to the filter output to receive the message,
        // pass in an anonymous function an parameter object
        let filter = Filter(name: "scale", output: PipeListener(context: self, listener: self.callBackMethod),
            filter: { (message: IPipeMessage, params: Any?) in
                let factor = (params as! [String: Int])["factor"]
                let rectangle = message.header as! Rectangle
                rectangle.width *= factor!
                rectangle.height *= factor!
        ...
        XCTAssertTrue((received.header as! Rectangle).height == 10, "Expecting received.header['height'] == 10")
    }
    ...
    */
    func testSetFilterByControlMessage() {
        ...
        XCTAssertTrue((received.header as! Rectangle).height == 0, "Expecting received.header['height'] == 0")
    }
