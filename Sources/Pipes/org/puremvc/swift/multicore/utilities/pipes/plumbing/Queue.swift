...
public class Queue: Pipe {
    ...
    private let messagesQueue = DispatchQueue(label: "org.puremvc.pipes.Queue.messagesQueue", attributes: DispatchQueue.Attributes.concurrent)
    ...
    */
    func store(_ message: IPipeMessage) {
        messagesQueue.sync(flags: .barrier, execute: {
            self.messages.append(message)
            if self.mode == QueueControlMessage.SORT {
                self.messages.sort(by: self.sortMessagesByPriority)
            }
        })
    }
