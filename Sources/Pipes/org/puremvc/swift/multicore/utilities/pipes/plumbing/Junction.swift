...
public class Junction {
    ...
    private let pipesMapQueue = DispatchQueue(label: "org.puremvc.pipes.Junction.pipesMapQueue", attributes: DispatchQueue.Attributes.concurrent)
    ...
    */
    public func registerPipe(_ name: String, type: String, pipe: IPipeFitting) -> Bool {
        var success = true
        pipesMapQueue.sync(flags: .barrier, execute: {
    ...
    */
    public func hasPipe(_ name: String) -> Bool {
        var result = false
        pipesMapQueue.sync {
    ...
    */
    public func hasInputPipe(_ name: String) -> Bool {
        var result = false
        pipesMapQueue.sync {
    ...
    */
    public func hasOutputPipe(_ name: String) -> Bool {
        var result = false
        pipesMapQueue.sync {
    ...
    */
    public func removePipe(_ name: String) {
        if self.hasPipe(name) {
            pipesMapQueue.sync(flags: .barrier, execute: {
                if let type = self.pipeTypesMap[name] {
                    ...
                    for (index, _) in pipesList.enumerated() {
                        if pipesList[index] == name {
                            pipesList.remove(at: index)
                            break
                        }
                    }
    ...
    */
    public func retrievePipe(_ name: String) -> IPipeFitting? {
    ...
    */
    public func addPipeListener(_ inputPipeName: String, context: AnyObject, listener: @escaping (IPipeMessage) -> Void) -> Bool {
        var success = false
        pipesMapQueue.sync {
    ...
    */
    public func sendMessage(_ outputPipeName: String, message: IPipeMessage) -> Bool { //read
        var success = false
        pipesMapQueue.sync {
